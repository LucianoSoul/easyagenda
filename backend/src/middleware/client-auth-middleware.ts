...
export async function clientAuthMiddleware(
  ...
  if (clientError) {
    throw new AppError(
      ...
      "Nao foi possivel carregar o contexto do cliente."
    );
  }

  const matchedClients = (clients ?? []) as ClientContextRow[];

  if (matchedClients.length === 0) {
    throw new AppError(
      403,
      "CLIENT_NOT_LINKED",
      "Cliente autenticado sem vinculo na aplicacao."
    );
  }
  ...
  if (matchedClients.length > 1) {
    throw new AppError(
      ...
      "Email do cliente vinculado a mais de um cadastro."
    );
  }
