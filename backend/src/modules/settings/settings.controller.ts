...
export async function registerSettingsRoutes(app: FastifyInstance) {
  ...
  app.patch("/", async (request) => {
    ...
    if (body.noShowAfterMinutes !== undefined) patch.no_show_after_minutes = body.noShowAfterMinutes;
    if (body.checkinOpensBeforeMinutes !== undefined) patch.checkin_opens_before_minutes = body.checkinOpensBeforeMinutes;
    if (body.checkinExpiresAfterMinutes !== undefined) patch.checkin_expires_after_minutes = body.checkinExpiresAfterMinutes;
    if (body.defaultPaymentGateway !== undefined) patch.default_payment_gateway = body.defaultPaymentGateway;
    if (body.pixKey !== undefined) patch.pix_key = body.pixKey;
    if (body.professionalProfile !== undefined) patch.professional_profile = body.professionalProfile;
