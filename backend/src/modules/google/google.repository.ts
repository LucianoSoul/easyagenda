...
import type {
  GoogleAppointmentEventMapping,
  GoogleAppointmentSyncContext,
  GoogleConnection,
  GoogleGatewaySettings,
  GoogleSettingsRow
...
function getGoogleGatewaySettings(
...
export class GoogleRepository {
  async getGoogleSettingsByUserId(userId: string) {
    ...
    const settings = data as GoogleSettingsRow;
    return {
      settings,
  ...
  async saveAppointmentEventMapping(input: {
    ...
    mapping: GoogleAppointmentEventMapping;
  }) {
    const current = await this.getGoogleSettingsByUserId(input.userId);
    const nextGatewaySettings = mergeGoogleGatewaySettings({
      currentGatewaySettings: current.settings.gateway_settings,
  ...
  async getAppointmentSyncContext(input: {
    appointmentId: string;
    accountId: string;
  }): Promise<GoogleAppointmentSyncContext | null> {
    const { data, error } = await supabaseAdmin
      .from("appointments")
    ...
    const appointment = data as GoogleAppointmentSyncContext & {
      clients: GoogleAppointmentSyncContext["clients"] | GoogleAppointmentSyncContext["clients"][];
      services: GoogleAppointmentSyncContext["services"] | GoogleAppointmentSyncContext["services"][];
      users: GoogleAppointmentSyncContext["users"] | GoogleAppointmentSyncContext["users"][];
      accounts: GoogleAppointmentSyncContext["accounts"] | GoogleAppointmentSyncContext["accounts"][];
    };
    ...
    const scheduling = getSchedulingGatewaySettings(
      (settingsData as { gateway_settings?: Record<string, unknown> | null }).gateway_settings
    );
