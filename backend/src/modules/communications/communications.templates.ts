import type {
  ...
  EmailCommunicationPayload,
  PushCommunicationPayload,
  WhatsAppCommunicationPayload
...
function buildEmail(to: string | null, subject: string, bodyLines: string[]): EmailCommunicationPayload {
  return {
    to,
    subject,
    body: bodyLines.join("\n")
...
function buildWhatsApp(to: string | null, textLines: string[]): WhatsAppCommunicationPayload {
  return {
    to,
    text: textLines.join(" ")
  };
...
function buildPush(title: string, body: string, deepLink: string): PushCommunicationPayload {
  return {
    title,
    body,
    deepLink
...
export function buildConsultationCreatedPendingPayment(
  context: ConsultationCommunicationContext
): CommunicationPayload {
  const message =
  ...
  return {
    ...
    push: buildPush(
      "Consulta criada",
      "Sua consulta foi criada e aguarda confirmacao do pagamento.",
      context.publicStatusUrl
    )
  };
}

export function buildPaymentApprovedInPerson(
  context: ConsultationCommunicationContext
): CommunicationPayload {
  return {
    ...
    push: buildPush(
      "Pagamento confirmado",
      "Sua consulta presencial foi confirmada.",
      context.publicStatusUrl
    )
  };
}

export function buildPaymentApprovedOnlineWithoutMeeting(
  context: ConsultationCommunicationContext
): CommunicationPayload {
  return {
    ...
    push: buildPush(
      "Pagamento confirmado",
      "Sua consulta online foi confirmada. O link sera liberado assim que estiver disponivel.",
      context.publicStatusUrl
    )
  };
}

export function buildPaymentApprovedOnlineWithMeeting(
  context: ConsultationCommunicationContext
): CommunicationPayload {
  ...
  return {
    ...
    push: buildPush(
      "Link da consulta liberado",
      "Sua consulta online esta confirmada e o link ja esta disponivel.",
      meetingLink
    )
  };
}

export function buildConsultationReminderInPerson(
  context: ConsultationCommunicationContext
): CommunicationPayload {
  return {
    ...
    push: buildPush(
      ...
      "Sua consulta presencial esta se aproximando.",
      context.publicStatusUrl
    )
  };
}

export function buildConsultationReminderOnline(
  context: ConsultationCommunicationContext
): CommunicationPayload {
  const deliveryLine =
    context.meeting?.meetLink || context.meeting?.htmlLink
  ...
  return {
    ...
    push: buildPush(
      "Lembrete da consulta",
      "Sua consulta online esta se aproximando.",
      context.publicStatusUrl
    )
  };
}
