...
const mapCheckinError = (message: string) => {
...
export class CheckinService {
  ...
  async performByToken(token: string) {
    ...
    const attendance = await this.repository.getLatestAttendanceByAppointmentId(
      usedToken.appointment_id
    );
    ...
    if (!attendance) {
      throw new AppError(
        ...
        "Check-in realizado, mas a presenca nao foi encontrada."
      );
    }
