...
function pickSingle<T>(value: RelationValue<T>): T | null {
...
function normalizeAppointmentRow(row: AppointmentWithRelationsRow): AppointmentWithRelationsRow {
  return {
    ...row,
    clients: pickSingle(row.clients as RelationValue<ClientRecord>),
    services: pickSingle(row.services as RelationValue<ServiceRecord>),
    users: pickSingle(row.users as RelationValue<ProviderRecord>),
    accounts: pickSingle(row.accounts as RelationValue<AccountRecord>)
  };
}
...
export class ClientRepository {
  async findClientByEmail(email: string): Promise<ClientRecord[]> {
  ...
  async listNextAppointments(input: {
    ...
    if (error) throw error;
    return ((data ?? []) as unknown as AppointmentWithRelationsRow[]).map(
  ...
  async listHistory(input: {
    ...
    if (error) throw error;
    return {
      data: ((data ?? []) as unknown as AppointmentWithRelationsRow[]).map(
        normalizeAppointmentRow
      ),
      count: count ?? 0
    };
  ...
  async getAppointmentDetail(input: {
    ...
    if (error) throw error;
    return data
      ? normalizeAppointmentRow(data as unknown as AppointmentWithRelationsRow)
      : null;
  }
}
