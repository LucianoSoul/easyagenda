...
import { buildConsultationCreatedPendingPayment, buildConsultationReminderInPerson, buildConsultationReminderOnline, buildPaymentApprovedInPerson, buildPaymentApprovedOnlineWithMeeting, buildPaymentApprovedOnlineWithoutMeeting } from "./communications.templates.js";
export class CommunicationsService {
    consultationsRepository;
    constructor(consultationsRepository = new ConsultationsRepository()) {
    ...
    async buildPaymentApprovedPayload(input) {
        ...
        if (context.paymentStatus !== "approved") {
            throw new AppError(409, "CONSULTATION_NOT_PAYABLE", "Consulta ainda nao possui pagamento aprovado.");
        }
    ...
    async getCommunicationsPreview(input) {
        ...
        return {
            ...
            currentlyRelevant: {
                consultation_created_pending_payment: context.paymentStatus === "pending"
                    ? buildConsultationCreatedPendingPayment(context)
                    : undefined,
                payment_approved: context.paymentStatus === "approved"
                    ? await this.buildPaymentApprovedPayload(input)
                    : null,
                consultation_reminder_in_person: context.deliveryMode === "in_person"
                    ? buildConsultationReminderInPerson(context)
                    : undefined,
                consultation_reminder_online: context.deliveryMode === "online"
                    ? buildConsultationReminderOnline(context)
                    : undefined
            }
        };
    }
    async getContextForAccount(input) {
        ...
        if (!consultation) {
            ...
            if (exists) {
                throw new AppError(403, "CONSULTATION_ACCOUNT_MISMATCH", "Consulta nao pertence a conta autenticada.");
            }
        ...
        const googleEvent = payment.status === "approved" && consultation.delivery_mode === "online"
        ...
        return {
            ...
            meeting: googleEvent && payment.status === "approved" && consultation.delivery_mode === "online"
                ? {
