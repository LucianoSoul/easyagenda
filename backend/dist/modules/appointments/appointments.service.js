...
import { getAppointmentDeliverySettings, getServiceAttendanceMode, isAppointmentDeliveryMode } from "../../shared/utils/delivery-modes.js";
export class AppointmentsService {
    appointmentsRepository;
    paymentsService;
    refundsRepository;
    constructor(appointmentsRepository = new AppointmentsRepository(), paymentsService = new PaymentsService(), refundsRepository = new RefundsRepository()) {
    ...
    async createAppointment(input) {
        ...
        if (requestedDeliveryModeInput !== undefined &&
            !isAppointmentDeliveryMode(requestedDeliveryModeInput)) {
            throw new AppError(400, "INVALID_DELIVERY_MODE", "deliveryMode deve ser in_person ou online.");
        }
        const requestedDeliveryMode = requestedDeliveryModeInput;
        let deliveryMode;
        if (attendanceMode === "hybrid") {
            if (!requestedDeliveryMode) {
                throw new AppError(400, "DELIVERY_MODE_REQUIRED", "Servico hibrido exige escolha entre atendimento presencial ou online.");
            }
            deliveryMode = requestedDeliveryMode;
        }
        else if (attendanceMode === "online_only") {
            if (requestedDeliveryMode === "in_person") {
                throw new AppError(400, "SERVICE_ATTENDANCE_MODE_MISMATCH", "Servico configurado como online_only aceita apenas agendamentos online.");
            }
            deliveryMode = "online";
        }
        else {
            if (requestedDeliveryMode === "online") {
                throw new AppError(400, "SERVICE_ATTENDANCE_MODE_MISMATCH", "Servico configurado como in_person_only aceita apenas agendamentos presenciais.");
            }
    ...
    async listAppointments(accountId) {
