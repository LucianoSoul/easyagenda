...
export class RefundsService {
    ...
    async processDecision(input) {
        ...
        if (decision.account_id !== input.accountId) {
            throw new AppError(403, "REFUND_DECISION_ACCOUNT_MISMATCH", "Decisao de reembolso nao pertence a conta autenticada.");
        }
        if (decision.decision_status !== "pending" ||
            decision.execution_status !== "not_processed") {
            throw new AppError(400, "REFUND_DECISION_ALREADY_PROCESSED", "Decisao de reembolso nao esta pendente para processamento.");
        }
        ...
        if (refundAmount === 0 && creditAmount === 0) {
            await this.runWithFailureHandling({
                ...
                operation: () => this.refundsRepository.markNoActionNeeded({
                    decisionId: decision.id,
                    appliedBy: input.userId
                })
            });
        ...
        if (creditAmount > 0) {
            await this.runWithFailureHandling({
                ...
                operation: () => this.refundsRepository.createCreditFromRefundDecision({
                    decisionId: decision.id,
                    appliedBy: input.userId
                })
            });
            ...
            if (!credit) {
                throw new AppError(500, "CREDIT_NOT_FOUND", "Credito nao encontrado apos o processamento da decisao.");
            }
            return {
                refundDecision,
                credit
        ...
        if (refundAmount > 0) {
            ...
            await this.runWithFailureHandling({
                ...
                appliedBy: input.userId,
                gatewayResponse,
                operation: () => this.refundsRepository.markProcessed({
                    decisionId: decision.id,
                    appliedBy: input.userId,
        ...
        throw new AppError(400, "REFUND_DECISION_INVALID_AMOUNTS", "Decisao de reembolso com valores invalidos para processamento.");
    }
    async getRequiredDecisionById(decisionId) {
    ...
    async runWithFailureHandling(input) {
        try {
            ...
            throw new AppError(500, "REFUND_DECISION_PROCESSING_FAILED", "Nao foi possivel processar a decisao de reembolso.");
        }
    }
    async tryMarkFailed(input) {
        try {
